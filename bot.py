...
"""
Loan Collection Voice Agent - PipeCat Bot Pipeline with Flows (FIXED)
=======================================================================
A structured loan collection conversation using Pipecat Flows.
Demonstrates: greeting → identity check → overdue info → understand situation
→ payment options → commitment → promise to pay → close.
...
FIXES:
- Proper transcript capture using Flow Manager event handlers
- Real-time metrics tracking from context aggregator
- Flow node tracking integrated with Flow Manager lifecycle
"""
...
# pc_id -> {current_node, metrics, start_time, tts_type, transcript, _context}
session_data: dict = {}
...
async def run_bot(webrtc_connection: SmallWebRTCConnection, tts_type: str = "deepgram"):
    ...
    # --- Transcript monitor (single processor for both user and assistant) ---
    transcript_monitor = TranscriptMonitor(pc_id)
    ...
    pipeline = Pipeline(
        [
            transport.input(),
            stt,
            transcript_monitor,  # Monitor all frames for transcript capture
            context_aggregator.user(),
            llm,
            tts,
            transport.output(),
